...
uint32_t sensedPoints[2][2]; // this is reading as bytes
...
//   for (int i = 0; i < 4; i++) {
//    if (MPR121.isNewTouch(i)) { // this is letting the board know if i is being touch from 0-4 electrodes then write "i is touched"
//      Serial.print("electrode ");
//      Serial.print(i, DEC);
//      Serial.println(" was just touched");
//    } else if (MPR121.isNewRelease(i)) { // else it reads that its not being touched anymore
//      Serial.print("electrode ");
//      Serial.print(i, DEC);
...
void printTable()
{
    ...
    for(int i=0; i<6e; i++)
    {
        for(int j=0; j<6; j++)
        {
            if(MPR121.isNewTouch(j != 6)) Serial.print(String(sensedPoints[i][j]) + " | ");
...
//    for(uint8_t i=0; i<2; i++) // means a byte 
//    {
//        for(uint8_t j=0; j<2; j++) sensedPoints[i][j] = cap_sense.filteredData(i) * cap_sense.filteredData(j + 6);
//    }
